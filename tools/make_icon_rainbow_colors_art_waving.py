...
# ---- Handgezeichnet wirkende Bandgrenzen ----
...
def handdrawn(n, amp):
    """
    Unregelmäßige, handgezeichnete Linie:
      - grobe Welle (niedrige Frequenz)
      - mittlere Welle
      - feines Zittern
      - gelegentliche scharfe Knicke
    """
    t = np.linspace(0, 1, n, dtype=np.float32)

    # Drei Zufallswellen mit unterschiedlicher Glättung
    coarse = smooth(rng.normal(0, 1, n).astype(np.float32), 160)
    medium = smooth(rng.normal(0, 1, n).astype(np.float32), 40)
    fine   = smooth(rng.normal(0, 1, n).astype(np.float32), 10)

    # Normieren auf [-1, 1]
    def norm(a): return a / (np.abs(a).max() + 1e-6)
    coarse, medium, fine = norm(coarse), norm(medium), norm(fine)

    # Zusätzlich ein paar Sinuswellen mit zufälliger Phase
    sine = np.zeros(n, dtype=np.float32)
    for _ in range(3):
        f = rng.uniform(2.0, 6.0)         # Frequenz
        p = rng.uniform(0, 2 * np.pi)     # Phase
        a = rng.uniform(0.3, 0.8)         # Amplitude
        sine += a * np.sin(2 * np.pi * f * t + p)
    sine = norm(sine)

    # Kombinieren: grob dominiert, fein gibt Textur
    v = 0.55 * coarse + 0.25 * medium + 0.10 * fine + 0.35 * sine

    # Gelegentliche scharfe Knicke: ~6 zufällige Stellen mit kleinem Sprung
    n_kinks = 6
    for _ in range(n_kinks):
        k = rng.integers(20, n - 20)
        width = rng.integers(2, 6)        # Sprungbreite in Pixeln
        jump = rng.uniform(-0.6, 0.6)
        v[k:k + width] += jump

    return (norm(v) * amp).astype(np.float32)

OFFSET_Y = handdrawn(H, 70.0)[:, None]   # Form (H, 1)
OFFSET_X = handdrawn(W, 70.0)[None, :]   # Form (1, W)
...
dated = f"../assets/icon/app_icon_rainbow_colors_art_waving_{stamp}.png"
